...
public class NitoriEntity extends Animal {
	public NitoriEntity(EntityType<NitoriEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
		...
		this.moveControl = new MoveControl(this) {
			@Override
			public void tick() {
				if (NitoriEntity.this.isInWater())
					NitoriEntity.this.setDeltaMovement(NitoriEntity.this.getDeltaMovement().add(0, 0.005, 0));
				if (this.operation == MoveControl.Operation.MOVE_TO && !NitoriEntity.this.getNavigation().isDone()) {
					double dx = this.wantedX - NitoriEntity.this.getX();
					double dy = this.wantedY - NitoriEntity.this.getY();
					double dz = this.wantedZ - NitoriEntity.this.getZ();
					float f = (float) (Mth.atan2(dz, dx) * (double) (180 / Math.PI)) - 90;
					float f1 = (float) (this.speedModifier * NitoriEntity.this.getAttribute(Attributes.MOVEMENT_SPEED).getValue());
					NitoriEntity.this.setYRot(this.rotlerp(NitoriEntity.this.getYRot(), f, 10));
					NitoriEntity.this.yBodyRot = NitoriEntity.this.getYRot();
					NitoriEntity.this.yHeadRot = NitoriEntity.this.getYRot();
					if (NitoriEntity.this.isInWater()) {
						NitoriEntity.this.setSpeed((float) NitoriEntity.this.getAttribute(Attributes.MOVEMENT_SPEED).getValue());
						float f2 = -(float) (Mth.atan2(dy, (float) Math.sqrt(dx * dx + dz * dz)) * (180 / Math.PI));
						f2 = Mth.clamp(Mth.wrapDegrees(f2), -85, 85);
						NitoriEntity.this.setXRot(this.rotlerp(NitoriEntity.this.getXRot(), f2, 5));
						float f3 = Mth.cos(NitoriEntity.this.getXRot() * (float) (Math.PI / 180.0));
						NitoriEntity.this.setZza(f3 * f1);
						NitoriEntity.this.setYya((float) (f1 * dy));
					} else {
						NitoriEntity.this.setSpeed(f1 * 0.05F);
					}
				} else {
					NitoriEntity.this.setSpeed(0);
					NitoriEntity.this.setYya(0);
					NitoriEntity.this.setZza(0);
				}
			}
		};
	}
	...
	protected void dropCustomDeathLoot(ServerLevel serverLevel, DamageSource source, boolean recentlyHitIn) {
		...
		this.spawnAtLocation(new ItemStack(FumoModItems.NITORI_ITEM.get()));
	}
	...
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		NitoriEntity retval = FumoModEntities.NITORI.get().create(serverWorld);
