...
public class NazrinEntity extends Animal {
	public NazrinEntity(EntityType<NazrinEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
		...
		this.moveControl = new MoveControl(this) {
			@Override
			public void tick() {
				if (NazrinEntity.this.isInWater())
					NazrinEntity.this.setDeltaMovement(NazrinEntity.this.getDeltaMovement().add(0, 0.005, 0));
				if (this.operation == MoveControl.Operation.MOVE_TO && !NazrinEntity.this.getNavigation().isDone()) {
					double dx = this.wantedX - NazrinEntity.this.getX();
					double dy = this.wantedY - NazrinEntity.this.getY();
					double dz = this.wantedZ - NazrinEntity.this.getZ();
					float f = (float) (Mth.atan2(dz, dx) * (double) (180 / Math.PI)) - 90;
					float f1 = (float) (this.speedModifier * NazrinEntity.this.getAttribute(Attributes.MOVEMENT_SPEED).getValue());
					NazrinEntity.this.setYRot(this.rotlerp(NazrinEntity.this.getYRot(), f, 10));
					NazrinEntity.this.yBodyRot = NazrinEntity.this.getYRot();
					NazrinEntity.this.yHeadRot = NazrinEntity.this.getYRot();
					if (NazrinEntity.this.isInWater()) {
						NazrinEntity.this.setSpeed((float) NazrinEntity.this.getAttribute(Attributes.MOVEMENT_SPEED).getValue());
						float f2 = -(float) (Mth.atan2(dy, (float) Math.sqrt(dx * dx + dz * dz)) * (180 / Math.PI));
						f2 = Mth.clamp(Mth.wrapDegrees(f2), -85, 85);
						NazrinEntity.this.setXRot(this.rotlerp(NazrinEntity.this.getXRot(), f2, 5));
						float f3 = Mth.cos(NazrinEntity.this.getXRot() * (float) (Math.PI / 180.0));
						NazrinEntity.this.setZza(f3 * f1);
						NazrinEntity.this.setYya((float) (f1 * dy));
					} else {
						NazrinEntity.this.setSpeed(f1 * 0.05F);
					}
				} else {
					NazrinEntity.this.setSpeed(0);
					NazrinEntity.this.setYya(0);
					NazrinEntity.this.setZza(0);
				}
			}
		};
	}
	...
	protected void dropCustomDeathLoot(ServerLevel serverLevel, DamageSource source, boolean recentlyHitIn) {
		...
		this.spawnAtLocation(new ItemStack(FumoModItems.NAZRIN_ITEM.get()));
	}
	...
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		NazrinEntity retval = FumoModEntities.NAZRIN.get().create(serverWorld);
