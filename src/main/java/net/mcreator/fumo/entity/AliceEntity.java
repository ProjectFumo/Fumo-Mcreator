...
public class AliceEntity extends Animal {
	public AliceEntity(EntityType<AliceEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
		...
		this.moveControl = new MoveControl(this) {
			@Override
			public void tick() {
				if (AliceEntity.this.isInWater())
					AliceEntity.this.setDeltaMovement(AliceEntity.this.getDeltaMovement().add(0, 0.005, 0));
				if (this.operation == MoveControl.Operation.MOVE_TO && !AliceEntity.this.getNavigation().isDone()) {
					double dx = this.wantedX - AliceEntity.this.getX();
					double dy = this.wantedY - AliceEntity.this.getY();
					double dz = this.wantedZ - AliceEntity.this.getZ();
					float f = (float) (Mth.atan2(dz, dx) * (double) (180 / Math.PI)) - 90;
					float f1 = (float) (this.speedModifier * AliceEntity.this.getAttribute(Attributes.MOVEMENT_SPEED).getValue());
					AliceEntity.this.setYRot(this.rotlerp(AliceEntity.this.getYRot(), f, 10));
					AliceEntity.this.yBodyRot = AliceEntity.this.getYRot();
					AliceEntity.this.yHeadRot = AliceEntity.this.getYRot();
					if (AliceEntity.this.isInWater()) {
						AliceEntity.this.setSpeed((float) AliceEntity.this.getAttribute(Attributes.MOVEMENT_SPEED).getValue());
						float f2 = -(float) (Mth.atan2(dy, (float) Math.sqrt(dx * dx + dz * dz)) * (180 / Math.PI));
						f2 = Mth.clamp(Mth.wrapDegrees(f2), -85, 85);
						AliceEntity.this.setXRot(this.rotlerp(AliceEntity.this.getXRot(), f2, 5));
						float f3 = Mth.cos(AliceEntity.this.getXRot() * (float) (Math.PI / 180.0));
						AliceEntity.this.setZza(f3 * f1);
						AliceEntity.this.setYya((float) (f1 * dy));
					} else {
						AliceEntity.this.setSpeed(f1 * 0.05F);
					}
				} else {
					AliceEntity.this.setSpeed(0);
					AliceEntity.this.setYya(0);
					AliceEntity.this.setZza(0);
				}
			}
		};
	}
	...
	protected void dropCustomDeathLoot(ServerLevel serverLevel, DamageSource source, boolean recentlyHitIn) {
		...
		this.spawnAtLocation(new ItemStack(FumoModItems.ALICE_ITEM.get()));
	}
	...
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		AliceEntity retval = FumoModEntities.ALICE.get().create(serverWorld);
