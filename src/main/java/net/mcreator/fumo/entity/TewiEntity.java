...
public class TewiEntity extends Animal {
	public TewiEntity(EntityType<TewiEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
		...
		this.moveControl = new MoveControl(this) {
			@Override
			public void tick() {
				if (TewiEntity.this.isInWater())
					TewiEntity.this.setDeltaMovement(TewiEntity.this.getDeltaMovement().add(0, 0.005, 0));
				if (this.operation == MoveControl.Operation.MOVE_TO && !TewiEntity.this.getNavigation().isDone()) {
					double dx = this.wantedX - TewiEntity.this.getX();
					double dy = this.wantedY - TewiEntity.this.getY();
					double dz = this.wantedZ - TewiEntity.this.getZ();
					float f = (float) (Mth.atan2(dz, dx) * (double) (180 / Math.PI)) - 90;
					float f1 = (float) (this.speedModifier * TewiEntity.this.getAttribute(Attributes.MOVEMENT_SPEED).getValue());
					TewiEntity.this.setYRot(this.rotlerp(TewiEntity.this.getYRot(), f, 10));
					TewiEntity.this.yBodyRot = TewiEntity.this.getYRot();
					TewiEntity.this.yHeadRot = TewiEntity.this.getYRot();
					if (TewiEntity.this.isInWater()) {
						TewiEntity.this.setSpeed((float) TewiEntity.this.getAttribute(Attributes.MOVEMENT_SPEED).getValue());
						float f2 = -(float) (Mth.atan2(dy, (float) Math.sqrt(dx * dx + dz * dz)) * (180 / Math.PI));
						f2 = Mth.clamp(Mth.wrapDegrees(f2), -85, 85);
						TewiEntity.this.setXRot(this.rotlerp(TewiEntity.this.getXRot(), f2, 5));
						float f3 = Mth.cos(TewiEntity.this.getXRot() * (float) (Math.PI / 180.0));
						TewiEntity.this.setZza(f3 * f1);
						TewiEntity.this.setYya((float) (f1 * dy));
					} else {
						TewiEntity.this.setSpeed(f1 * 0.05F);
					}
				} else {
					TewiEntity.this.setSpeed(0);
					TewiEntity.this.setYya(0);
					TewiEntity.this.setZza(0);
				}
			}
		};
	}
	...
	protected void dropCustomDeathLoot(ServerLevel serverLevel, DamageSource source, boolean recentlyHitIn) {
		...
		this.spawnAtLocation(new ItemStack(FumoModItems.TEWI_ITEM.get()));
	}
	...
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		TewiEntity retval = FumoModEntities.TEWI.get().create(serverWorld);
