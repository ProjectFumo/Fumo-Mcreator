...
public class YuyukoEntity extends Animal {
	public YuyukoEntity(EntityType<YuyukoEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
		...
		this.moveControl = new MoveControl(this) {
			@Override
			public void tick() {
				if (YuyukoEntity.this.isInWater())
					YuyukoEntity.this.setDeltaMovement(YuyukoEntity.this.getDeltaMovement().add(0, 0.005, 0));
				if (this.operation == MoveControl.Operation.MOVE_TO && !YuyukoEntity.this.getNavigation().isDone()) {
					double dx = this.wantedX - YuyukoEntity.this.getX();
					double dy = this.wantedY - YuyukoEntity.this.getY();
					double dz = this.wantedZ - YuyukoEntity.this.getZ();
					float f = (float) (Mth.atan2(dz, dx) * (double) (180 / Math.PI)) - 90;
					float f1 = (float) (this.speedModifier * YuyukoEntity.this.getAttribute(Attributes.MOVEMENT_SPEED).getValue());
					YuyukoEntity.this.setYRot(this.rotlerp(YuyukoEntity.this.getYRot(), f, 10));
					YuyukoEntity.this.yBodyRot = YuyukoEntity.this.getYRot();
					YuyukoEntity.this.yHeadRot = YuyukoEntity.this.getYRot();
					if (YuyukoEntity.this.isInWater()) {
						YuyukoEntity.this.setSpeed((float) YuyukoEntity.this.getAttribute(Attributes.MOVEMENT_SPEED).getValue());
						float f2 = -(float) (Mth.atan2(dy, (float) Math.sqrt(dx * dx + dz * dz)) * (180 / Math.PI));
						f2 = Mth.clamp(Mth.wrapDegrees(f2), -85, 85);
						YuyukoEntity.this.setXRot(this.rotlerp(YuyukoEntity.this.getXRot(), f2, 5));
						float f3 = Mth.cos(YuyukoEntity.this.getXRot() * (float) (Math.PI / 180.0));
						YuyukoEntity.this.setZza(f3 * f1);
						YuyukoEntity.this.setYya((float) (f1 * dy));
					} else {
						YuyukoEntity.this.setSpeed(f1 * 0.05F);
					}
				} else {
					YuyukoEntity.this.setSpeed(0);
					YuyukoEntity.this.setYya(0);
					YuyukoEntity.this.setZza(0);
				}
			}
		};
	}
	...
	protected void dropCustomDeathLoot(ServerLevel serverLevel, DamageSource source, boolean recentlyHitIn) {
		...
		this.spawnAtLocation(new ItemStack(FumoModItems.YUYUKO_ITEM.get()));
	}
	...
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		YuyukoEntity retval = FumoModEntities.YUYUKO.get().create(serverWorld);
