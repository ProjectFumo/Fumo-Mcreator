...
public class PatchouliSpawnProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if ((world.getBlockState(BlockPos.containing(x, y + 1, z))).getBlock() == Blocks.AIR || (world.getBlockState(BlockPos.containing(x, y + 1, z))).getBlock() instanceof LiquidBlock) {
			if (entity instanceof Player _player) {
				ItemStack _stktoremove = new ItemStack(FumoModItems.PATCHOULI_ITEM.get());
			...
			if (world instanceof ServerLevel _level)
				_level.getServer().getCommands().performPrefixedCommand(
						...
						("/summon fumo:patchouli ~ ~ ~ " + "{Rotation:[" + (entity.getYRot() - 180) + "f]}"));
		}
	}
}
